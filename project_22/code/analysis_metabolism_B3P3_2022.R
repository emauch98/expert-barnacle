################################################################################
# This metabolism analysis of Pond B3P3 is meant to serve as a blueprint for future
# anlyses regarding the ponds from the Greenland expeditions of EAWAG
#
# Aim of this sript: model ecosystem metabolism of B3P3. The whole workflow is
# described in the corresponding report.
#
# Model used: Bayesian
#
# Observation period used to estimate metabolism of B3P3:
...
k600.vachon <- k.vachon(all.600, lake.area = 3000)
...
################################################################################
# The max depth of pond B3P3 is about 5m.
# According to the profiling plots, B3P3 was fully stratified on June 21 2022
# (mixed layer depth < 0.5m)
# Since mixed layer depth is highly dependent on weather, I'll asssume an
# average mixed layer depth of 2m over the whole observation period
################################################################################
...
ols.res <- metab(all, method = "ols", wtr.name='wtr', do.obs.name='do.obs',
                      irr.name="par", z.mix = "z.mix", do.sat.name = "do.sat",
                 k.gas.name = "k.gas")
fwrite(ols.res, "../results_metabolism/B3P3_ols_2022.txt", sep="\t")

# MLE model:
mle.res <- metab(all, method = "mle", wtr.name='wtr', do.obs.name='do.obs',
                 irr.name="par", z.mix = "z.mix", do.sat.name = "do.sat",
                 k.gas.name = "k.gas")
fwrite(mle.res, "../results_metabolism/B3P3_mle_2022.txt", sep="\t")

# MLE + Kalman filter:
kalman.res <- metab(all, method='kalman', wtr.name='wtr', do.obs.name='do.obs',
                    irr.name='par', z.mix = "z.mix", do.sat.name = "do.sat",
                 k.gas.name = "k.gas")
fwrite(kalman.res, "../results_metabolism/B3P3_kalman_2022.txt", sep="\t")
...
fwrite(bayesian.merge, "../results_metabolism/B3P3_bayesian_2022.txt", sep="\t")
...
fig3 <- annotate_figure(fig2, top = text_grob("Parameters used to estimate metabolism of B3P3 in 2022"))
...
# Create a date variable
ols.res$Date <- as.Date(paste(ols.res$year, "-", ols.res$doy, sep = ""), format = "%Y-%j")

# Select columns to keep
ols.res <- ols.res[,c("Date", "GPP", "R", "NEP", "Model")]

mle.res$Model <- "MLE"

# Create a date variable
mle.res$Date <- as.Date(paste(mle.res$year, "-", mle.res$doy, sep = ""), format = "%Y-%j")

# Select columns to keep
mle.res <- mle.res[,c("Date", "GPP", "R", "NEP", "Model")]

kalman.res$Model <- "MLE + Kalman filter"

# Create a date variable
kalman.res$Date <- as.Date(paste(kalman.res$year, "-", kalman.res$doy, sep = ""), format = "%Y-%j")

# Select columns to keep
kalman.res <- kalman.res[,c("Date", "GPP", "R", "NEP", "Model")]
...
# Save
fwrite(merged.res, "../results_metabolism/merged_results_B3P3_2022.txt", sep="\t")
...
# to get the maximum amount of information out of our data.
