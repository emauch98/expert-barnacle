################################################################################
# This is a metabolism analysis of pond B2P4 in 2023
# Treatment B2P4: no Fish
...
meta <- meta %>%
  mutate(datetime = as.POSIXct(format(datetime, format = "%Y-%m-%d %H:04:00", tz = "UTC"), tz = "UTC"))
...
k600.vachon <- k.vachon(all.600, lake.area = 5100) # From LakeMaster file
...
fwrite(bayesian.merge, "../results_metabolism/B2P4_bayesian_2023.txt", sep="\t")
...
fig3 <- annotate_figure(fig2, top = text_grob("Parameters used to estimate metabolism of B2P4 in 2023"))

plot(fig3)

# Save the plot as a .png
png("../results_metabolism/parameters_B2P4_2023.png", width = 1200, height = 900)
print(fig3)
dev.off()
...
fig4 <- annotate_figure(fig1, top = text_grob("Estimated metabolism of pond B2P4 in 2023"))
plot(fig4)

# Save the plot as a .png
png("../results_metabolism/metabolism_B2P4_2023.png", width = 1200, height = 900)
print(fig4)
dev.off()
