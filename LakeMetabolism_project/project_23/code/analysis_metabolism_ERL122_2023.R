################################################################################
# This is a metabolism analysis of pond ERL122 in 2023
# Treatment ERL152: no Fish
# Observation period used to estimate metabolism:
# 2023-06-23 00:00:00 to 2023-07-17 00:00:00
# Model used: Bayesian
################################################################################
...
meta <- meta %>%
  mutate(datetime = as.POSIXct(format(datetime, format = "%Y-%m-%d %H:12:00", tz = "UTC"), tz = "UTC"))
...
meta_ext <- meta %>%
  complete(datetime = seq(from = min(datetime), to = max(datetime), by = "15 min"))
...
# Perform a full outer join, because a few rows in "all" are missing
# (the sondes did not measure for a short period on 2023-06-29)
# I can Impute those values later
all <- full_join(meta_ext, all, by = "datetime")
...
# Make a timeseries object for both DO and water temperature
ts <- ts(all$do.obs, frequency = 96)
ts2 <- ts(all$wtr, frequency = 96)
...
imp_all2 <- na_ma(ts2) # use this function!
imp_all2 <- data.table(imp_all2)
imp_all2$datetime <- all$datetime
...
all$wtr <- imp_all2$imp_all2

colSums(is.na(all)) # The missing Pond character values don't matter for estimation
...
k600.vachon <- k.vachon(all.600, lake.area = 11000) # From LakeMaster file
...
# According to the profiling plots, ERL122 had an actively mixed layer depth of
# about 1.75m on Sept 16 2021
# Since mixed layer depth is highly dependent on weather, I will assume an
# average mixed layer depth of 2 m over the whole period
################################################################################
...
fwrite(bayesian.merge, "../results_metabolism/ERL122_bayesian_2023.txt", sep="\t")
...
fig3 <- annotate_figure(fig2, top = text_grob("Parameters used to estimate metabolism of ERL122 in 2023"))
...
fig4 <- annotate_figure(fig1, top = text_grob("Estimated metabolism of pond ERL122 in 2023"))
plot(fig4)

# Save the plot as a .png
png("../results_metabolism/metabolism_ERL122_2023.png", width = 1200, height = 900)
print(fig4)
dev.off()
