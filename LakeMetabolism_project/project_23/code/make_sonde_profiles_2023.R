...
system.time(for(i in filenames){
  ...
  # Extract pond names
  if (grepl("BeachLake_6m", i)) {
    dummy2$Pond <- "BeachLake_6m"
  } else if (grepl("BeachLake_shallowend", i)) {
    dummy2$Pond <- "BeachLake_shallowend"
  } else {
    dummy2$Pond <- sub("_.*", "", i)
  }
...
all <- all[,c("DateMMDDYYYY", "TimeHHmmss", "Pond", "Depthm", "ChlorophyllRFU", "CondScm", "SpCondScm",
              "fDOMRFU", "ODOmgL", "ODOsat", "pH", "TempC")]
...
all <- all %>%
  mutate(
    # Extract month and day from Date_time and format them
    Day = format(Date_time, "%B %d")
  )
...
setnames(all, c("Date_time", "Pond", "Day", "Depth_m", "Chlorophyll_RFU", "Cond_uScm", "SpCond_uScm",
                "fDOM_RFU", "ODO_mgL", "ODO_sat", "pH", "Temp_C"))
...
thresholds <- c("BeachLake_6m"=0.52, "BeachLake_shallowend"=0.52, "ERL01"=0.52, "ERL125"=0.6,
                "ERL137"=0.75, "ERL138"=0.52, "ERL153"=0.52, "ERL16"=0.52, "ERL17"=0.52,
                "ERL33"=0.52, "ERL60"=0.52, "ERL65"=0.52, "ERL67"=0.52, "ERL84"=0.52)
...
# save
fwrite(all, "../data/sondes_profiling_cut/merged_profiles_cut_2023.txt", sep = "\t")
...
plots_TempC <- lapply(ponds, function(pond_name) {
  ...
  ggplot(data = dat, aes(x = Temp_C, y = Depth_m)) +
    ...
    geom_path() +
    scale_y_reverse() +
    labs(title = paste("Thermocline", pond_name, ",", day, "2023"))
})

# Save the plot as a .png
png("../results_profiling/profiling_TempC_2023.png", width = 1800, height = 1400)
...
plots_Chloro <- lapply(ponds, function(pond_name) {
  ...
  ggplot(data = dat, aes(x = Chlorophyll_RFU, y = Depth_m)) +
    ...
    geom_path() +
    scale_y_reverse() +
    labs(title = paste("Chlorophyll RFU", pond_name, ",", day, "2023"))
})

# Save the plot as a .png
png("../results_profiling/profiling_ChloroRFU_2023.png", width = 1800, height = 1400)
...
plots_fDOM <- lapply(ponds, function(pond_name) {
  ...
  ggplot(data = dat, aes(x = fDOM_RFU, y = Depth_m)) +
    ...
    geom_path() +
    scale_y_reverse() +
    labs(title = paste("fDOM RFU", pond_name, ",", day, "2023"))
})

# Save the plot as a .png
png("../results_profiling/profiling_fDOM_2023.png", width = 1800, height = 1400)
...
plots_ODO <- lapply(ponds, function(pond_name) {
  ...
  ggplot(data = dat, aes(x = ODO_mgL, y = Depth_m)) +
    ...
    labs(title = paste("Dissolved Oxygen mg/L", pond_name, ",", day, "2023"))
})

# Save the plot as a .png
png("../results_profiling/profiling_ODO_2023.png", width = 1800, height = 1400)
