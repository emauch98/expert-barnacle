################################################################################
# This is a metabolism analysis of pond B1P1 in 2023
...
meta <- meta %>%
  mutate(datetime = as.POSIXct(format(datetime, format = "%Y-%m-%d %H:03:00", tz = "UTC"), tz = "UTC"))
...
k600.vachon <- k.vachon(all.600, lake.area = 2100) # From LakeMaster file
...
# For 2023, we do't have profiling data. I'll assume that the average mixed
# layer depth is about 2m for the whole period
################################################################################
...
fwrite(bayesian.merge, "../results_metabolism/B1P1_bayesian_2023.txt", sep="\t")

# Step 11: Visualize the results -----------------------------------------------

# Plots of ecosystem parameters-------------------------------------------------

# Define a common theme to use for all plots
...
p5 = ggplot((all)) +
  ...
  rremove("xlab") +
  ylab(expression(paste("Equilibrium DO (mg L",NULL^-1,")"))) +
  xlab("Date") +
  common_theme
...
p7 = ggplot((all)) +
  ...
  rremove("xlab") +
  ylab(expression(paste("Irradiation (",mu,"mol m",NULL^-2," s",NULL^-1,")"))) +
  xlab("Date") +
  common_theme
...
# Combine plots with shared theme
fig2 <- ggarrange(p4, p5, p6, p7, p8, nrow = 5)

# fig3 <- annotate_figure(fig2, top = text_grob("Parameters used to estimate metabolism of B1P1 in 2023"))

plot(fig2)

# Save the plot as a .png
png("../results_metabolism/parameters_B1P1_2023.png", width = 2200, height = 1300)
print(fig2)
dev.off()
...
fig4 <- annotate_figure(fig1, top = text_grob("Estimated metabolism of pond B1P1 in 2023"))
plot(fig4)

# Save the plot as a .png
png("../results_metabolism/metabolism_B1P1_2023.png", width = 1200, height = 900)
print(fig4)
dev.off()
