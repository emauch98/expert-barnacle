...
The data preparation for estimating lake metabolism was done separately for each monitoring
period (16 September-24 September 2021, 22 June-3 July 2022, and 22 June-17 July 2023).

\subsubsection*{Sonde data}

First, the raw sonde data from each lake were imported and merged into
one dataset for a given monitoring period. The variables needed for lake metabolism
estimation were water temperature (\textdegree{}C) and
dissolved oxygen (mg/L). Next, an outlier analysis was performed by removing
...
After outlier removal, the time series of water temperature and dissolved oxygen
were subsequently investigated for larger data gaps due to potential lack of sensor
...
observation period. Due to large data gaps during the period of 2022, we chose
...
After cutting, missing values were replaced by a
weighted moving average, where the weights of observations around the
central value decrease exponentially \citep{Moritz:2017}.
For larger data gaps spanning over several hours, we chose
to not estimate lake metabolism for the affected days, rather than imputing these values.
