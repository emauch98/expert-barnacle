################################################################################
# This is a metabolism analysis of pond B1P4 in 2023
# Treatment B1P4: no Fish
# Observation period used to estimate metabolism:
# 2023-06-23 00:00:00 to 2023-07-17 00:00:00
# Model used: Bayesian
# This pond has been called B3P4 in the raw data file
################################################################################
...
meta <- meta %>%
  mutate(datetime = as.POSIXct(format(datetime, format = "%Y-%m-%d %H:02:00", tz = "UTC"), tz = "UTC"))
...
################################################################################
# The max depth of pond B1P4 is about 2.2m.
# Since mixed layer depth is highly dependent on weather, I'll assume an
# average mixed layer depth of 1.5m over the whole period
################################################################################
...
fwrite(bayesian.merge, "../results_metabolism/B1P4_bayesian_2023.txt", sep="\t")
...
fig3 <- annotate_figure(fig2, top = text_grob("Parameters used to estimate metabolism of B1P4 in 2023"))

# Save the plot as a .png
png("../results_metabolism/metabolism_B1P4_2023.png", width = 1200, height = 900)
plot(fig3)
dev.off()
...
fig4 <- annotate_figure(fig1, top = text_grob("Estimated metabolism of pond B1P4 in 2023"))
plot(fig4)
